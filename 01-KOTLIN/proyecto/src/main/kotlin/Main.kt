...
fun main(){
    ...
    val sumarCuatro = Suma(null,null)
    sumaUno.sumar();
    sumaDos.sumar();
    sumaTres.sumar();
    sumarCuatro.sumar();
    ...
    arregloEstatico.forEachIndexed{ indice: Int, valorActual: Int->println("Valor ${valorActual} indice: ${indice}")}
    ...
    val respuestaMap: List<Double> = arregloDinamico.map{ valorActual: Int ->
        return@map valorActual.toDouble() + 100.10 //return@OPERACION - devuelve a la funcion map y cuando termine de
        //iterar, nos devuelve una lista de doubles.
    }
    ...
    //1) Devolver una expresion (TRUE or FALSE) por cada iteracion (TODOS LOS TRUE ENTRAR EN EL ARREGLO)
    //2) Nuevo arreglo filtrado

    val respuestaFilter: List<Int> = arregloDinamico.filter {valorActual: Int
    -> /*CONDICION*/ val mayoresACinco: Boolean = valorActual > 5
    ...
    val respuestaReduce: Int = arregloDinamico.reduce{
        //acumulado = 0 -> SIEMPRE EMPIEZA en 0
...
class Suma( // Constructor Primario Suma
    ...
    public fun sumar(): Int { //No toma parámetros y devuelve un valor entero
